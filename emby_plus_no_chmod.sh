...
if [ $1 ]; then
	...
	case $cpu_arch in
		"x86_64" | *"amd64"*)
			docker run -d --name emby -v $1/config:/config -v $1/xiaoya:/media --net=host --user 3000:3000 --restart always emby/embyserver:4.8.0.56
			...
			;;
		"aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
		        docker run -d --name emby -v $1/config:/config -v $1/xiaoya:/media --net=host  --user 3000:3000 --restart always emby/embyserver_arm64v8:4.8.0.56
			#docker cp /tmp/Emby.Server.Implementations.dll emby:/system/
                        #docker exec -i emby chmod 644 /system/Emby.Server.Implementations.dll
                        #docker restart emby
			echo "一键全家桶全部安装完成"
